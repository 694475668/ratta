...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid')
				.datagrid(
						{
							url : '${pageContext.request.contextPath}/dictionaryController/dataGrid',
							method : 'post',
							//idField : 'code',
							pageSize : 10,
							...
							columns : [ [
									...
									{
										field : 'remark',
										title : '<spring:message code="bz_code_name"/>',
										width : 150,
										align : 'center',
										halign : 'center'
									},
									...
									{
										field : 'value_cn',
										title : '<spring:message code="code_name_cn"/>',
										width : 110,
										align : 'center',
										halign : 'center'
									},
									{
										field : 'value_en',
										title : '<spring:message code="code_name_en"/>',
										width : 250,
										align : 'center',
										halign : 'center'
									},
	...
	function addFun() {
		parent.$
				.modalDialog({
					...
					height : 350,
					//method: 'get',
					href : '${pageContext.request.contextPath}/dictionaryController/addDictionary',
					buttons : [ {
	...
	function editFun() {
		...
		parent.$
				.modalDialog({
					...
					width : 600,
					height : 350,
					href : '${pageContext.request.contextPath}/dictionaryController/editDictionary?id='
							+ row.id,
					buttons : [ {
						text : '<spring:message code="save"/>',
				...
				.confirm(
						...
						function(b) {
							if (b) {
								...
								$
										.post(
												'${pageContext.request.contextPath}/dictionaryController/delete',
												{
													id : row.id
												},
												function(result) {
	...
	<div id="toolbar" style="display: none;">
		<a onclick="searchFun();" href="javascript:void(0);"
			...
			data-options="iconCls:'brick_delete',plain:true"><spring:message
				code="clear" /></a>
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/dictionaryController/addDictionary')}">
		...
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/dictionaryController/editDictionary')}">
		...
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/dictionaryController/delete')}">
