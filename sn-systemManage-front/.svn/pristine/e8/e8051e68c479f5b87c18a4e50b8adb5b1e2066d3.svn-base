...
$.fn.panel.defaults.onBeforeDestroy = function() {
	...
	try {
		if (frame.length > 0) {
			...
			frame.remove();
			if (navigator.userAgent.indexOf("MSIE") > 0) {// IE特有回收內存方法
				try {
					CollectGarbage();
				} catch (e) {
				}
 ...
 * 通用錯誤提示
 * 
 * 用於datagrid/treegrid/tree/combogrid/combobox/form加載數據出錯時的操作
 */
 ...
 * @requires jQuery,EasyUI
 * 
 * 為datagrid、treegrid增加表頭菜單，用於顯示或隱藏列，註意：凍結列不在此菜單中
 */
var createGridHeaderContextMenu = function(e, field) {
	e.preventDefault();
	var grid = $(this);/* grid本身 */
	var headerContextMenu = this.headerContextMenu;/* grid上的列頭菜單對象 */
...
/**
 * Datagrid擴展方法tooltip 基於Easyui 1.3.3，可用於Easyui1.3.3+
 * 
 * 簡單實現，如需高級功能，可以自由修改
 * 
 * 使用說明:
 * 
 * 在easyui.min.js之後導入本js
 * 
 * 代碼案例:
 * 
...
/**
 * Treegrid擴展方法tooltip 基於Easyui 1.3.3，可用於Easyui1.3.3+
 * 
 * 簡單實現，如需高級功能，可以自由修改
 * 
 * 使用說明:
...
$.extend($.fn.tree.methods, {
	getCheckedExt : function(jq) {// 獲取checked節點(包括實心)
	...
	getSolidExt : function(jq) {// 獲取實心節點
		var checked = [];
		var checkbox2 = $(jq).find("span.tree-checkbox2").parent();
		$.each(checkbox2, function() {
			var node = $.extend({}, $.data(this, "tree-node"), {
 ...
 * 創建壹個模式化的dialog
 * 
 * @returns $.modalDialog.handler 這個handler代表彈出的dialog句柄
 * 
 * @returns $.modalDialog.xxx 這個xxx是可以自己定義名稱，主要用在彈窗關閉時，刷新某些對象的操作，可以將xxx這個對象預定義好
 */
$.modalDialog = function(options) {
	if ($.modalDialog.handler == undefined) {// 避免重復彈出
		var opts = $.extend({
			title : '',
			width : 840,
			height : 680,
